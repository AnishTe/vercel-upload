...
import { AlertCircle, FileX, ShoppingBag } from "lucide-react";
import {
  Select,
  SelectContent,
  SelectItem,
...
import { columns, type BuybackHOEntry } from "./columns";
import {
  BuildingIcon,
  LibraryIcon,
} from "lucide-react";
...
export default function BuybackHOPlaceOrders() {
  const [data, setData] = useState<BuybackHOEntry[]>([]);
  ...
  const [selectedRows, setSelectedRows] = useState<BuybackHOEntry[]>([]);
  ...
  const fetchBuybackData = useCallback(async (isin: string) => {
    ...
    setError(null);
    try {
      const response = await BuybackHO({ isin: isin });
      const tokenIsValid = validateToken(response);
      if (!tokenIsValid) {
        setShowSessionExpiredModal(true);
        return;
  ...
  const handleConfirm = async () => {
    ...
    try {
      const record = selectedRows.map((row) => ({
        applied: row.inputs.quantity || row.applied, // Use applyQuantity if available, otherwise fallback to applied
        clientId: row.clientId.toString(),
        quantity: row.quantity,
        boid: row.boid,
        scrip: row.scrip,
        isin: row.isin,
        buybackid: selectedBuybackId, // Use the selectedBuybackId instead of row.buybackId
        branchcode: row.branchcode.toString(),
        orderType: "CDSL"
      }));

      const response = await applyforBuybackHO({ data: record });
      const tokenIsValid = validateToken(response);
      ...
      if (responseData && responseData.data) {
        ...
        // Custom JSX content for multiline toast
        const toastContent = (
          <div>

...
import { ongoingBuyback, buybackOrderbookMismatch } from "@/lib/auth"
...
import { columns, type BuybackOrderBookMismatch } from "./columns"
import { CalendarIcon, BuildingIcon, DollarSignIcon, BarChartIcon, TrendingUpIcon, LibraryIcon } from "lucide-react"
...
export default function Buyback() {
  const [data, setData] = useState<BuybackOrderBookMismatch[]>([])
  ...
  const fetchBuybackData = useCallback(async (isin: string, buybackId: string) => {
    setLoading(true)
    setError(null)
    try {
      const response = await buybackOrderbookMismatch({ isin: isin, buybackid: buybackId })
      const tokenIsValid = validateToken(response)
      if (!tokenIsValid) {
        setShowSessionExpiredModal(true)
        return
  ...
  const handleCompanySelect = (value: string) => {
    ...
    if (selectedCompany) {
      ...
      fetchBuybackData(selectedCompany.isin, selectedCompany.value)
    }
  }
    ...
    <DashboardLayout>
      <Card className="w-full">
        <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-2 sm:space-y-0">
          <CardTitle>Buyback OrderBook Mismatch</CardTitle>
        </CardHeader>
        <CardContent>

...
import {
  ...
  applyforNSDLBuybackHO,
  deleteNSDLBuybackOrdersHO,
...
import { columns, type NSDLBuybackOrdersHO } from "./columns"
...
export default function NSDLBuyback() {
  const [activeTab, setActiveTab] = useState("orders")
  const [data, setData] = useState<NSDLBuybackOrdersHO[]>([])
  ...
  const [selectedRows, setSelectedRows] = useState<NSDLBuybackOrdersHO[]>([])
  const [generateFileModalOpen, setGenerateFileModalOpen] = useState(false)
  const [deleteConfirmDialogOpen, setDeleteConfirmDialogOpen] = useState(false)
  const [orderToDelete, setOrderToDelete] = useState<NSDLBuybackOrdersHO | null>(null)
  ...
  // const handleSelectedRowsChange = useCallback((rows: NSDLBuybackOrdersHO[]) => {
  ...
  const fetchBuybackData = useCallback(async (isin: string) => {
    ...
    setError(null)
    try {
      const response = await getNSDLBuybackOrders({ isin })
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const response = await applyforNSDLBuybackHO({ data: [orderToConfirm] })
      const tokenIsValid = validateToken(response)
  ...
  const handleDeleteOrders = useCallback(async () => {
    ...
    try {
      ...
      // Make the API call
      const response = await deleteNSDLBuybackOrdersHO({ data: records })
  ...
  const handleGenerateFile = async (data: { exchange: "NSE" | "BSE" }) => {
    try {
      setLoading(true)
      const response = await generateNSDLBuybackFile({
        isin: selectedCompanyDetails?.isin || "",

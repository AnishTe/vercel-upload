...
import { columns, type BuybackOrderBookDetailsBranch } from "./columns";
import {
  CalendarIcon,
  BuildingIcon,
  DollarSignIcon,
...
export default function BuybackBranckOrderBook() {
  const [data, setData] = useState<BuybackOrderBookDetailsBranch[]>([]);
  ...
  const fetchBuybackData = useCallback(async (isin: string) => {
    ...
    setError(null);
    try {
      const response = await getOrderbookDetailsBranch({ isin: isin });
      const tokenIsValid = validateToken(response);
      if (!tokenIsValid) {
        setShowSessionExpiredModal(true);
        return;

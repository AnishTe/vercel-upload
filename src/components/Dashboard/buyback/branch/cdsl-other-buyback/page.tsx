...
import { columns, type CDSLOtherBuybackOrdersBranch } from "./columns"
...
export default function CDSLOtherBuyback() {
  ...
  const [data, setData] = useState<CDSLOtherBuybackOrdersBranch[]>([])
  ...
  const [selectedRows, setSelectedRows] = useState<CDSLOtherBuybackOrdersBranch[]>([])
  // const [generateFileModalOpen, setGenerateFileModalOpen] = useState(false)
  const [deleteConfirmDialogOpen, setDeleteConfirmDialogOpen] = useState(false)
  const [orderToDelete, setOrderToDelete] = useState<CDSLOtherBuybackOrdersBranch | null>(null)
  ...
  const fetchBuybackData = useCallback(async (isin: string) => {
    ...
    setError(null)
    try {
      const response = await getCDSLOtherBuybackOrdersBranch({ isin })
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const response = await applyforCDSLOtherBuybackBranch({ data: [orderToConfirm] })
      const tokenIsValid = validateToken(response)
  ...
  const handleDeleteOrders = useCallback(async () => {
    ...
    try {
      ...
      // Make the API call
      const response = await deleteCDSLOtherBuybackOrdersBranch({ data: records })

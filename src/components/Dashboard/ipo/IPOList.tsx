...
import { ipoDetails } from "@/lib/auth"
import { useIPO } from "@/contexts/IPOContext"
import { Badge } from "@/components/ui/badge"
import { Filter, ArrowUpDown, AlertCircle, Package } from "lucide-react"
...
export default function IPOList({
    ...
    useEffect(() => {
        ...
        if (filterExchange !== "all") {
            result = result.filter((ipo) => ipo.companylogoex === filterExchange)
        }
    ...
    }, [groupedIpoData, sortBy, filterExchange, searchTerm])
    ...
    const handleViewHistory = async (isin: string) => {
        ...
        if (selectedIpoData) {
            ...
            if (selectedCategoryData) {
                try {
                    ...
                    // Now navigate
                    if (openViewHistoryRoute && loginType === "Branch") {
                        router.push(getCompatibleUrl(`${openViewHistoryRoute}?ipoId=${selectedCategoryData.ipoid}`))
                    } else {
                        router.push(getCompatibleUrl(`/client/dashboard/ipo/applyIPO?ipoId=${selectedCategoryData.ipoid}`))
                    }
                } catch (error) {
                    console.error("Error setting IPO data:", error)
                    toast.error("Failed to prepare IPO history view. Please try again.")
                }
            } else {
    ...
    const handleApplyIPO = async (isin: string) => {
        const selectedIpoData = groupedIpoData.find((ipo) => ipo.isin === isin)
        if (!selectedIpoData) {
            toast.error("IPO data not found. Please try again.")
            return
        }

        const selectedCategoryData = selectedIpoData.categories.find(
            (c: any) => c.category === selectedIpoData.selectedCategory,
        )
        ...
        try {
            // First, set the data in context and wait for it to complete
            await setSelectedIPO({
                ...selectedIpoData,
                ipoid: selectedCategoryData.ipoid,
                category: selectedCategoryData.category,
            ...
            const storedData = localStorage.getItem("selectedIPO")
            if (!storedData) {
                throw new Error("Failed to store IPO data")
            }

            // Now navigate to the appropriate route
            const url = openIPORoute
                ? getCompatibleUrl(`${openIPORoute}?ipoId=${selectedCategoryData.ipoid}`)
                : getCompatibleUrl(`/client/dashboard/ipo/applyIPO?ipoId=${selectedCategoryData.ipoid}`)

            router.push(url)
        } catch (error) {
            console.error("Error setting IPO data:", error)
            toast.error("Failed to prepare IPO application. Please try again.")
        }
    }

    const handleUpdateDetailsIPO = (isin: string) => {
        const selectedIpoData = groupedIpoData.find((ipo) => ipo.isin === isin)
        if (selectedIpoData) {
            const selectedCategoryData = selectedIpoData.categories.find(
                (c) => c.category === selectedIpoData.selectedCategory,
            ...
            if (selectedCategoryData) {
                ...
                router.push(`/branch/dashboard/ipo/update-details?ipoId=${selectedCategoryData.ipoid}`)
            } else {
                toast.error("Invalid IPO category selected. Please try again.")
            }
        }
    ...
    const uniqueExchanges = Array.from(new Set(groupedIpoData.map((ipo) => ipo.companylogoex)))
                                    ...
                                    {loading ? (
                                        ...
                                        <>
                                            ...
                                            <div className="mt-3 pt-2 border-t">
                                                {ipolisttype === "all" && accountType === "employee" && (
                                                    <Button
                                                        className="w-full bg-teal-500 hover:bg-teal-600 transition-all duration-300"
                                                        onClick={() => handleUpdateDetailsIPO(ipo.isin)}
                                                    >
                                                        Update Details
                                                    </Button>
                                                )}

                                                {ipolisttype === "ongoing" && loginType === "Branch" && (
                                                    <div className="flex items-center gap-2">
                                                        <Button
                                                            className="flex-1 bg-teal-500 hover:bg-teal-600 transition-all duration-300"
                                                            onClick={() => handleApplyIPO(ipo.isin)}
                                                        >
                                                            Apply for Client
                                                        </Button>
                                                        <Button
                                                            ...
                                                            onClick={() => handleViewHistory(ipo.isin)}
                                                        >
                                                            View History
                                                        </Button>
                                                    </div>
                                                ...
                                                {ipolisttype === "closed" && loginType === "Branch" && (
                                                    <Button
                                                        className="w-full bg-teal-500 hover:bg-teal-600 transition-all duration-300"
                                                        onClick={() => handleViewHistory(ipo.isin)}
                                                    >
                                                ...
                                                {ipolisttype === "ongoing" && loginType === "Client" && (
                                                    <Button
                                                        className="w-full bg-teal-500 hover:bg-teal-600 transition-all duration-300"
                                                        onClick={() => handleApplyIPO(ipo.isin)}
                                                    >

...
import { applyMtfPledgeHO, mtfPledgeHO } from "@/api/auth"
...
import { columns, type MTFPledgeHOEntry } from "./columns"
...
export default function MTFPledgeHO() {
  ...
  const [selectedRows, setSelectedRows] = useState<MTFPledgeHOEntry[]>([])
  ...
  const fetchData = useCallback(async (id: string) => {
    ...
    setError(null)
    try {
      const response = await mtfPledgeHO({ clientId: id.trim() })
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const response = await applyMtfPledgeHO({ record: record })
      const tokenIsValid = validateToken(response)
            ...
            <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
              <div className="flex items-center gap-2 w-full sm:w-auto">
                ...
                <div className="flex items-center gap-2 flex-1">
                  ...
                  />
                  <Button onClick={fetchMTFPledgeData} size="sm" className="h-9 px-2 shrink-0">

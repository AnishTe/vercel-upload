...
import { columns, type MarginPledgeBranchEntry } from "./columns";
...
export default function MarginPledge() {
  ...
  const [selectedRows, setSelectedRows] = useState<MarginPledgeBranchEntry[]>(
    []
  );
  ...
  const fetchData = useCallback(async (id: string) => {
    ...
    setHasSearched(true);
    try {
      const response = await marginPledgeBranch({ clientId: id.trim() });
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const response = await applyMarginPledgeBranch({ record: record });
      const tokenIsValid = validateToken(response);
      if (!tokenIsValid) {
        setShowSessionExpiredModal(true);
        return;
  ...
  useEffect(() => {
    ...
    const updateButtons = () => {
      ...
      const allRowNodes = tableRef.current
        ? $(tableRef.current).find("tr, tr.child")
        : $();
      ...
      $(allRowNodes).each(function () {
        ...
        const isSelected =
          ...
          (isResponsive && prevRow.hasClass("selected"));
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="w-1/4 text-right">Segment</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {selectedRows && selectedRows.length > 0 ? (
                    selectedRows.map((share, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell className="teSxt-right">
                          {share.inputs?.segment || "-"}
                        </TableCell>
                      </TableRow>
                    ))
                  ) : (
      ...
      <Dialog open={showApplyModal} onOpenChange={setShowApplyModal}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This will apply the segment <strong>{currentSegment}</strong> to
              all {selectedRows.length} selected rows.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>

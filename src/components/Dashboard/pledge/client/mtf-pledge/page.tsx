...
import { mtfPledge, applyMtfPledge } from "@/api/auth"
...
import type { MTFPledgeEntry } from "./columns"
...
export default function MTFPledge() {
  const [data, setData] = useState<MTFPledgeEntry[]>([])
  const [originalData, setOriginalData] = useState<MTFPledgeEntry[]>([])
  ...
  const fetchMtfPledgeData = async () => {
    ...
    setError(null)
    try {
      const response = await mtfPledge()
      ...
      setData(parsedData)
      setOriginalData(parsedData) // Store the original data order
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const response = await applyMtfPledge({ record: record })
      const tokenIsValid = validateToken(response)
  ...
  // const handleQuantityChange = (itemKey: string, value: string, maxQuantity: number) => {
  //   // Force integer value by removing any decimal part
  //   const intValue = Number.parseInt(value, 10)
  ...
  const handleCheckboxChange = (checked: boolean, item: MTFPledgeEntry) => {
  ...
  const renderListItem = (item: MTFPledgeEntry, index: number) => {
    const itemKey = `${item.boID}-${item.isin}`
    const errorKey = `${itemKey}-quantity`
            ...
            <div className="grid grid-cols-12 gap-4 items-center">
              ...
              <div className="col-span-4 flex items-center justify-end">
                <div className="flex flex-col items-center">
                  <span className="text-[10px] font-medium text-gray-700">Pledge Qty</span>
                  <QuantityInput
                    itemKey={itemKey}
                    currentValue={currentValue}
                    hasError={hasError}
            ...
            <>
              {data.length === 0 ? (
                ...
                <div className="space-y-1" ref={tableRef}>
                  {/* Select All Row with Sorting Dropdown */}
                  <div className="flex items-center justify-between p-2 bg-muted/10 rounded-md mb-3 shadow-sm gap-2">
                    ...
                    {/* Sorting Dropdown - Positioned at the right end */}
              ...
              <Table>
                ...
                <TableBody>
                  {selectedRows && selectedRows.length > 0 ? (
                    ...
                    <TableRow>
                      <TableCell colSpan={2} className="text-center py-4 text-xs">
                        No scripts selected
                      </TableCell>
                    </TableRow>
                  )}
